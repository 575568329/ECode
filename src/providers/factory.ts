 ...
 * 流程：model → ModelResolution（查 providerKey）→ ProviderConfig（查 protocol/baseURL/apiKeyEnv）
...
export function createProvider(model: string): ModelProvider {
  const providerKey = getModelConfig(model).providerKey;
  ...
  if (!apiKey) {
    throw new Error(
      `模型 ${model} 缺少 API Key。请在 ~/.ecode/config.json 的 providers.${providerKey}.apiKey 填入（推荐，全局可用），或设置环境变量 ${providerConfig.apiKeyEnv}。`,
    );
  }
