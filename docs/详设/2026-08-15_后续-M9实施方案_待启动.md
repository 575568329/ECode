...
- **Tab 专职档位热键**（2026-08-16 拍板）：Tab 在主输入空闲态与 /sandbox 面板内直接循环切档（default → read-only → workspace-write → full-access → 环绕），StatusBar 闪示当前档；进 full-access 弹确认（M9-D12）；输入焦点/搜索态不拦截。**代价与清偿**：Tab 原三份职责全部让位——①全局焦点循环（输入框↔折叠区）撤销，焦点态收敛为「输入框 ↔ 面板/弹窗」两种；②工具块展开从「Tab 聚焦+Enter」两步并为 Ctrl+O 一步（本就是全局 toggle）；③ ask_user 多问 chip 导航改 ←→（QuestionPanel 无 tabs 不冲突）——键位规范同步改
...
| M9-D13 | 档位切换键位 | **Tab 专职**（主输入空闲态+面板内循环切档）；原焦点循环撤销（收敛为输入框↔面板两态）、工具展开并为 Ctrl+O、ask_user chips 改 ←→ | 2026-08-16 用户拍板"Tab 直接切换、其他职责去掉"；Tab 语义唯一可记（档位=模式），焦点循环是低频且已有 Ctrl+O 平替 |
