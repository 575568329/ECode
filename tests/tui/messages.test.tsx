...
describe('UserMessage', () => {
  ...
  it('F-36 栅格（2026-08-29 用户拍板）：❯ 严格顶第 0 列，续行对齐内容列（图标槽 2 → 第 2 列起），不顶格', () => {
    const { lastFrame } = render(React.createElement(UserMessage, { text: '第一行\n第二行' }))
    const f = lastFrame() ?? ''
    const lines = f.split('\n')
    const first = lines.find((l) => l.includes('第一行')) ?? ''
    const second = lines.find((l) => l.includes('第二行')) ?? ''
    expect(first.startsWith('❯')).toBe(true) // 首行：❯ 严格第 0 列（背景块无左 padding）
    expect(second.startsWith('  ')).toBe(true) // 续行：对齐内容列第 2 列，不回第 0 列
  })
})
