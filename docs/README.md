...
| 后续 M6 实施方案 | [详设/2026-08-14_后续-M6实施方案_待审核.md](./详设/2026-08-14_后续-M6实施方案_待审核.md) | **v4（双触发面+按需加载拍板）：M6 = Skill + MCP 两子系统，Plugin 推迟 M7**（第三部分保留为 M7 设计储备）——Skill 三层渐进式加载 + **双触发面**（LLM SkillTool + `/skill args` 手动 `$ARGUMENTS` 传参，`user-invocable`/`disable-model-invocation` 双布尔独立开关）+ token 预算 + 蒸馏 L2（创建+**升级合并协议**：versions 备份+merger 三态+section 级合并）；MCP 官方 SDK + stdio&HTTP + **按需加载**（lazy 默认+metadata cache 解耦注册与连接+六态状态机+failed 60s 过期退避+空闲卸载）+ **`/mcp` 三层 TUI**（StatusBar/systemMsgs/命令）+ 默认确认 + 首用批准；v3 修订全保留（MCP 工具 token 计入压缩估算 / register 跳过外部 schema 预编译 / 盲确认修复 / M5 债 #3 DRY→S-P2）；全文第一人称重写，参考来源集中附录 B；心脏零改动经核实成立；M6-D24 不采纳 ADR-026 AppContext |
