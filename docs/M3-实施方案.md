...
> **前置**: M2 完成(Provider 抽象已落地,agent 已解耦 SDK)+ M3 前置借鉴点已做(①⑤⑧⑨,见 [00-开发规划](./00-开发规划.md) M3「前置准备」表)
> **参考来源**: [00-开发规划](./00-开发规划.md) M3 章节与认知盲区 P0-3/P1-1、[借鉴Vercel-AI-SDK对比报告](./借鉴Vercel-AI-SDK对比报告.md)、[M2-方案解析 §4.3 孤儿配对预告](./M2-方案解析.md)、**Claude Code 源码**(`D:\Study\claude-code-main`,2026-08-03 实地核查)、**CCode 源码**(`D:\Study\CCode`,9 条引用全部属实)、opencode Session 模型。所有"数值/实现"类断言均带 `file:line` 出处,见正文 📌 标记。
>
...
| 4 | **`providerOptions` 侧信道模式** | P1 格式 v2 的核心表达(中性骨架挂 provider 私有特性) | 🔴 无 | 看 [借鉴报告 §四⑥](./借鉴Vercel-AI-SDK对比报告.md) 已整理,实操落地 |
...
**关联文档**: `./M3-方案解析.md`(原理详解)、`./00-开发规划.md`(M3 章节)、`./借鉴Vercel-AI-SDK对比报告.md`(借鉴点来源)、`./M2-方案解析.md`(§4.3 孤儿配对预告)
**参考项目(均有源码出处)**:
