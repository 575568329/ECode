...
| 后续 Dogfood 实测修复方案 | [详设/2026-08-28_后续-Dogfood实测修复方案_待审核.md](./详设/2026-08-28_后续-Dogfood实测修复方案_待审核.md) | **v1.7 功能测试批完成（F-23~F-28）**：七批提交链（批1 4575e92→批2a F-18 根修→批2c be3ccd7→批2d 15b9957+9c87945 Notification+BEL→serve 事件驱动 ≈1.0×→批2b 9c12ab1 审批卡五条→批2b-fix 7b88970 审阅修复）；批2b 四角色审阅 §13.8/§13.8a（P0×5+偏差②裁决=明示口径；1449）；**功能测试批 §13.4a 六项全通**（MCP 全链/skill 双触发/auto-memory/四面板/CC 对比锚点 4/4）+F-24 skill junction 静默跳过+F-28 skillTool 注册表劈叉（makeSkillTool 工厂）已修，F-23 serve 命令分流/F-26 /output 裸 id 入档；效率基线 3-5×→1.3×→serve ≈1.0×+skill 沉淀 |
