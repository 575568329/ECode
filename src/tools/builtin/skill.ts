...
import { skillRegistry, type SkillRegistry } from "../../services/skill.js";
import { globalSkillHooks } from "../../services/hooks/global.js";
import type { ToolContext } from "../interface.js";

/** F-28（skill 注册表劈叉）：工厂注入 registry——serve freshRegistries 每项目新实例，
 *  静态单例闭包读不到 load 装入的内容（「Skill 不存在，可用：（无）」）。
 *  参照 makeTaskTool 先例：装配期创建注入；skillTool 默认导出绑全局单例保 REPL/argv 兼容。 */
export function makeSkillTool(reg: SkillRegistry): Tool {
  return {
    name: "Skill",
    description:
    ...
    input_schema: {
      ...
      properties: {
        skill: {
          type: "string",
          description: "Skill 名（available_skills 清单里的 name）",
        },
      },
      required: ["skill"],
    },
    ...
    async execute(args, ctx?: ToolContext) {
      ...
      if (ctx?.session?.isSkillActive?.(skill) === true) {
        return {
          content: `<skill_notice name="${skill}">该 skill 指令已在上文生效，无需重复加载。</skill_notice>`,
        };
      }
      const info = reg.get(skill);
      if (info === undefined) {
        const names = reg.listForPrompt().map((s) => s.name);
        return {
      ...
      if (info.hooks !== undefined && info.hooks.length > 0) {
        ...
        const summary = info.hooks
          .map(
            (h) =>
              `${h.event}${h.matcher !== undefined ? `(${h.matcher})` : ""}`,
          )
          .join("、");
        lines.push(
          "",
          `该 Skill 已启用 ${info.hooks.length} 个 hooks（本会话）：${summary}`,
        );
      }
...
/** 全局单例版（REPL/argv 兼容——assembly makeProjectParts 装配时用 makeSkillTool 注入项目实例）。 */
export const skillTool: Tool = makeSkillTool(skillRegistry);
