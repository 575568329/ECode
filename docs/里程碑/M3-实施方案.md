...
> **状态**: ⬜ 待审阅(本文档 + [M3-方案解析](./M3-方案解析.md) 确认后进入 TDD 编码)
> **前置**: M2 完成(Provider 抽象已落地,agent 已解耦 SDK)+ M3 前置借鉴点已做(①⑤⑧⑨,见 [00-开发规划](../总纲/00-开发规划.md) M3「前置准备」表)
> **参考来源**: [00-开发规划](../总纲/00-开发规划.md) M3 章节与认知盲区 P0-3/P1-1、[借鉴Vercel-AI-SDK对比报告](../调研/借鉴Vercel-AI-SDK对比报告.md)、[M2-方案解析 §4.3 孤儿配对预告](./M2-方案解析.md)、**Claude Code 源码**(`D:\Study\claude-code-main`,2026-08-03 实地核查)、**CCode 源码**(`D:\Study\CCode`,9 条引用全部属实)、opencode Session 模型。所有"数值/实现"类断言均带 `file:line` 出处,见正文 📌 标记。
>
> 📖 **先读[第零章总览](#零总览先看这里做什么--用什么--缺什么)**:做什么 / 用什么 / 缺哪些知识。
> 🔍 **想懂"为什么这么设计"看 [M3-方案解析](./M3-方案解析.md)**;本文是"怎么做"。
...
| 4 | **`providerOptions` 侧信道模式** | P1 格式 v2 的核心表达(中性骨架挂 provider 私有特性) | 🔴 无 | 看 [借鉴报告 §四⑥](../调研/借鉴Vercel-AI-SDK对比报告.md) 已整理,实操落地 |
...
- [ ] 读过 [M2-方案解析 §4.3](./M2-方案解析.md)(孤儿配对坑的事实依据)
...
- 详见 [M3-方案解析 §3.4](./M3-方案解析.md) 和本方案 [第五章](#五上下文压缩算法m3-核心)
...
> 这节是 M3 最难、最高风险的部分。原理详见 [M3-方案解析 §3](./M3-方案解析.md),这里讲**怎么做**。
...
**关联文档**: `./M3-方案解析.md`(原理详解)、`../总纲/00-开发规划.md`(M3 章节)、`../调研/借鉴Vercel-AI-SDK对比报告.md`(借鉴点来源)、`./M2-方案解析.md`(§4.3 孤儿配对预告)
**参考项目(均有源码出处)**:
