...
> **当前状态**：M1（心脏）+ M2（TUI 最小 Static）+ M3（工具集+ConfirmPrompt+bash 安全+.ecodeignore）+ M4（HistoryStore/向导/双协议 Provider）+ M5（上下文压缩：投影分离+分批 map-reduce 800k→200k 58s+cache 四维成本）+ M6（Skill 双触发面+MCP 按需加载+面板+内置 ecode-config 手册）+ M7（HookRunner 六事件+Plugin 安装链+PluginPanel）+ **M8（交互与上下文智能：ECODE.md 两级注入+截断可配/ask_user 选项框/auto-memory/WebFetch+SSRF/分段化/债清账+方案外交付：告警中心/面板+ /doctor 自检）全部完成**（2026-08-16 实施，852/852+两轮审阅修复；流式 markdown 彻底放弃 M8-D12）。**M9（安全网与质量闭环：hooks 接线修复 P0/checkpoint+`/rewind`（content-addressed 快照+区间跳过投影）/lint-test 回喂熔断/git 轻量（autoCommit+`/undo` trailer 校验）/沙箱四档+Tab 专职热键+blockedCommands/权限系统首步 Hook(owner) 三态三层/LSP 接口位）全部完成**（2026-08-16 实施，952/952+偏差与问题清单，`2026-08-15_后续-M9实施方案_已完成.md` 文末实施记录）→ **M10 = 感知扩展与配置可视化**（**全部完成** 2026-08-17 实施+终审修复+复审修复 1011/1011：多模态输入 ImageBlock+**PDF document block**（v1.2 扩入）：read_file 路径引用+双协议翻译 / 图片粘贴 P2 可选（Win/macOS）/ WebSearch 免费优先三层（搜索 MCP 优先→cn.bing RSS 零配置默认（唯一免费引擎，DDG 彻底放弃）→智谱可选；图片链路照做但**解析不兜底**（守卫只说"切模型或装图像 MCP"，不默认引导 GLM-4.6V-Flash/本地模型），v1.8；**2026-08-29 用户拍板拆视觉名门——isVisionModel/NO_VISION_MESSAGE 删除，read_file 图片恒直传由端点自证能力（glm-5.3-flash 有视觉被名单误拦真机实证，拆后真机读图通过）**） / /config 应用内面板：三页签+jsonc 非破坏修改+$EDITOR 逃生口 / 后台子进程任务 P3（v1.3 扩入：run_in_background+task_output/task_stop+统一杀树，顺带修 bash 单点 kill 孙进程泄漏）/ §9 滚动待办排期（M11 TUI 债候选——动态区高度预算系列 2026-08-18 拍板暂不执行），`2026-08-16_后续-M10实施方案_已完成.md` 文末实施记录）。**M11（Subagent+Todo+插话）全部完成**（2026-08-18 实施 P0-P7+实施记录，1043/1043：task 工具内嵌 runLoop 并发子代理（裁剪禁配 task/ask_user/todo+独立压缩链/独立 QualityGate/confirm 串行队列/transcript 独立+agentId 日志隔离/stop 谎报防御顺带修）+ todo 全量替换消息即状态+主循环插话（pollUserInput 步间注入+轮末兜底/Ctrl+U/F2 入队过 hook/图片标签免费随行），方案 v1.4（角色审阅修 F1-F4）（task 工具内嵌 runLoop 心脏零改动/并发直上 readonly 并行池/禁配 task+ask_user+todo/transcript 独立/日志 agentId/顺带修 stop 谎报；v1 经核码审阅修 3 P0——confirm 串行队列防死锁/独立压缩链防超窗炸父/makeTaskTool 工厂注入通路——+6 P1；**v1.2 扩入 todo 任务清单工具**（2026-08-18 用户拍板：builtin 静态+全量替换+readonly/消息即状态免 Store（rewind·恢复一致性免费）/transcript 内联 ASCII 状态符/子代理禁配取 opencode 紧口径）；**v1.3 扩入主循环插话**（2026-08-18 用户拍板：忙碌态输入激活+Enter 排队/双时点投递（loop pollUserInput 步间注入主路径+轮末兜底）/Ctrl+C 中断不弃队列（CC 同款）/Ctrl+U 清空/给子代理插话仍不做——跑完即返不变），`2026-08-17_后续-M11-Subagent实施方案_已完成.md` 文末实施记录+v1.4 变更记录，调研 `解析/2026-08-17_Subagent机制调研_已完成.md`）。2026-08-21 四角色全项目审阅修复批（P0×3：quality lintCommand 空串语义+自动探测 npm-scripts RCE 链断 / read_file·grep 共用敏感门（词法+realpath 双判防 symlink 穿透）/ 子代理桥 configRef 落实；P1×14+P2 若干：命令分词归一/连续同 role 合并/timeout_ms 兑现/retryable 消费/MCP env deny-list/SSRF 补段/undo trailer 精确/checkpoint 哈希复验等，1127/1127+tsc 净，提交 5e50543~6b1ea25 四笔）。 → **M12 = 服务化与多端：已完成（阶段 1-2 反转全批交付 + P0 token 统计补完；G1 真机点验待用户）**（调研 `解析/2026-08-18_M12服务化与多端调研_已完成.md` 含 2026-08-21 §8 补充：orca/harness 深挖 + spike 协议验证 8/8（可答帧多端收敛/断线重放/queue-steer/历史分页）+ opencode·codex·harness 三家分离架构实证；用户拍板：**多项目主机级 daemon**（web 统领所有项目，选项目即对话）+ **client/server 反转 TUI 先客户端化**（四阶段：协议收口（同进程直连）→daemon 化→Web→手机）+ 双入口 `ecode`/`ecode serve`（ecode-web 别名）；实施方案 `详设/2026-08-21_后续-M12-服务化与多端实施方案_已完成.md`：**M12 = token 统计（P0 独立可先行）+ 反转阶段 1-2**（协议双枚举+ClientContext transport 抽象+ApprovalBroker 挂起式审批+插话 TurnInputMode 协议化+TuiApp 客户端化+四桥实例化+命令 host 化→G1；daemon 入口/生命周期（opencode 蓝本）/ProjectRegistry 冷启动·回收·项目级互斥→G2），**Web/手机/IM 滑 M13**；Q1~Q13 决策台账+四条铁律+附录参考锚点/被否决备选；**v1.1（2026-08-21 用户拍板先做 TUI+基座）：§11 实施计划批次化，一批一提交一全量门；v1.2（三角色审阅修 5 P0）：§11.0 进程级状态全量盘点（补 globalExtensionHooks/activeAgents/skillRegistry/setWebSearch·FetchLimits/commandRegistry+工具层硬接线+makeDeps 隐式依赖）+ 新增 B8a 前置批（makeDeps 参数化：去 7 处 process.cwd/sessionId 分层/main() 接线宿主化）+ D5 会话上下文经 ToolContext + D6 审批分策略表（sensitive 永远交互、--yes 不覆盖，防 argv 密钥外泄面重开）+ §4.5 daemon token（M12 即做，防本机进程代答审批）与 loopback 判定要点 + 协议枚举补 error/usage 带 cost/systemMsg/askSelect 等 12 项 + B1 插话带图预组装 + 天估 20-26 天；§12 审阅记录**）。 **实施交付（2026-08-21~22）**：阶段 1-2 全批 B0→B9 十一笔提交链（协议地基/宿主会话/审批 Broker 挂起式/TuiApp 切协议客户端 1521→1310 行/会话级单例拆除/命令 host 化最小/HTTP serve+daemon token/makeDeps 参数化/ProjectRegistry 三道校验互斥/serveMulti 多项目+G2 双客户端同帧验收）+ 审阅修复批（P0×3：cwd 四处接线/Tab 沙箱真接宿主//rewind 宿主权威）+ daemon 生命周期增强（serve stop/接管/防脑裂 watchdog；三家调研结论：常驻无 idle 退出）+ **P0 token 统计补完（批1 `3ac3b80`：OpenAI input 去 cache 重复计价修复+压缩链 usage 上报（此前漏账）+recordUsage 三源收口；批2 `2348c7d`：UsageStatsRecord 追加式落盘（自包含 cwd/model——聚合不依赖 meta，旧文件兼容）+宿主累计器（四维+成本+**MCP 调用计数**（用户追加需求））/stats 命令（文件级 mtime 缓存+按天/模型/项目/最贵会话+缓存命中率）；1171+3skip）**；挂账 M13：B5 缩水四命令/B4b hooks 进程级/四注册表工厂化/权限 ask 桥多宿主串台（P1-10）/Web·手机·IM（M13 主体与 M13 成本优化方案并行待审）。**M13 前置拍板（2026-08-24 用户收窄需求）**：Web/移动端打开→看到本机哪些项目有历史→点项目新建/恢复对话→可输新项目路径开聊；明确不做活流回填与 TUI 跨进程——细化见 M12 方案 §15。**同日追加（§15.1+调研 `解析/2026-08-24_M13多会话并发与Web同步调研_已完成.md`）**：实证 harness/opencode 同项目多对话并发（会话级并发+项目级共享）与 mux 单流同步；M13 第一批扩为架构批 5-7 天：HostSession 拆 ProjectHost/ConversationHost 两层+ProjectRegistry 会话 Map+协议帧带 project/sessionId+/api/events.mux 单流+host 级生命周期帧；meta.cwd//api/projects 聚合不变，session/restore 升级为 ensure 会话级宿主。**M13 = 已实施（2026-08-24~25，18 批提交链 ee8b122→a2faac5，`详设/2026-08-24_后续-M13-实施方案_已完成.md` §13 实施记录）**：架构批（两层宿主 ProjectHost/ConversationHost+三全局单例收敛/MuxFrame 信封——内层枚举零改动/mux 单流+HostEvent 五帧+三预留/历史发现三件套→G-架构门✓）+Web/PWA（web/ 独立包 Vite8+React19.2+Tailwind4+Zustand5：连接工程 fetch+eventsource-parser 退避重连/daemon 托管 dist/两级列表/对话页展示·交互层/审批 composer-takeover sensitive 不给始终键/PWA 离线壳+移动两态+软键盘 --vvh）+飞书 IM gateway（WSClient 长连接免公网//new·/sessions·/switch 命令面/3s 回执/审批卡片三键+终态 patch，config.feishu 凭据激活）+成本线（B1 去重二连/B2 loopGuard+审批 15min 超时/B3 roles.summary 三项变更/B4 子代理超时自总结/B0 探针——**GLM 端点自动 KV 缓存实证，B5 打标取消**）；终态 118 文件 1217+3skip+tsc 净+web build 216KB；**G3 真机门/G-IM 真机门待用户实测**；挂账：Streamdown 接入位/飞书富文本群聊/model·set 顶栏。**2026-08-26 web 布局批（deepseek-harness AppFrame/常驻 composer 同款）**：输入区常驻主列底部全状态渲染（未选项目禁用占位/hero 态「输入即开新对话」/会话态插话——routing.ts 纯函数抽出+旧 new 占位退役）/侧栏「+」添加项目（POST /api/projects 校验+注册免 confirm，register 返回规范化路径）/「+新对话」真新建（session/new——serve 信封层拦截不走会话承载，经承载冷项目会多起空默认会话）/系统提示词 cwd 会话化（buildSystemPrompt opts.cwd——曾烤死 process.cwd()，多项目 agent 报错目录而 bash 实跑项目目录）/web 命令恒带 confirm=true（用户显式交互即栅栏同意语义——历史反推项目 428 静默死修复）/历史图片直渲（base64 data URI——restoreFull 已内联，零 serve 端点）/web 测试基建（vitest 21 测：store 状态机+routing；根仓 vitest.config.ts exclude web 防误拾）；Playwright MCP 全链路实测过（含真 LLM 轮回）；1263+3skip+web build 380KB(117KB gzip)。**2026-08-27 model·set 顶栏批（9fe109b，M13 挂账收尾）**：宿主接线 model/set+config/get（redact 广播/在册校验——此前 NOT_IMPLEMENTED 死分支）+ serve 每项目浅克隆 config（model/set 改活引用 current，共享对象跨项目串台——curl 实证隔离）/web 顶栏 ModelChip（桌面细条+移动并入返回行；config 视图随选中项目拉取、changed 帧只认选中项目）/移动端 390×844 两态复测过+新会话空态提示/**深链覆盖真 bug 修复**（hash→store 与 store→hash 两 effect 声明顺序对调——首帧把 null 选择态写回 hash，冷启动/刷新深链丢失）；1264+3skip+web 23 测+build 382KB。**同日楔死 P0 复测**：加压探针 scripts/pty-wedge-probe.cjs 入库（4 轮含 80-delta 长流+每轮末 stdin 回显探针判定调度活性）3 连跑全过+四场景探针 4/4——**未复现**（疑似 9ad1a83 轮末 setState 时序修复顺带治愈；源码机理未定位，待真机确认销案）。**2026-08-27 四角色审阅修复批**（四 agent 并行审 M12 后半/M13/Web ~6000 行未审面→报告 docs/解析/2026-08-27_四角色全项目代码审阅_修复中.md edceece→三批修复 3e28cec/2203e87/aaaa23d，1270+3skip）：P0×4 全修（飞书白名单 allowUsers 默认拒+卡片 operator 校验/web AbortController 迭代重建/命令管辖权三缺陷——session-list cwd 过滤+冷会话 restore 直通 ensureRestore 根治幻影/static stream error）；P1 组——acquire·ensureRestore 异常收敛、events catch+serve unhandledRejection、飞书审批带 sessionId+/new 真新建、mux 契约改真+created 补订 ev 流、补拉竞态并入、Rejected 保留输入；快赢——拒绝反馈喂回模型（approval/respond message 链路全通）、brief.title 截断、projects body cap、裸命令退役。余留挂账见报告进度节（规则引擎/审计落盘/事件游标三大项随 M14）。**2026-08-29 /restart 输入死根治**（用户真机报 npm run dev 重启后新实例无法输入）：pty-restart-probe v2 复现（旧 F-41 探针两盲点——非 dev 形态+回显失败只 WARN——假绿）→ 双进程 stdin 诊断日志定位：子进程 Ink 输入管线挂载完好（readable 监听 rdb=1/handle refd=true）却零字节，pause() 不取消内核里 pending 的 console ReadFile，输入永远先喂父句柄上那次读并被丢弃 → 修复=spawn 后 process.stdin.destroy() 丢弃副本句柄（对照实验 cmd /c npm run dev 重建链亦绿佐证死区在继承句柄）；探针入库作回归哨兵（回显死=exit 2）。**产品化定调（2026-08-24：奔产品非个人自用）——M14=配对+中继+E2EE+per-device token（orca 蓝本）+微信 ClawBot+企微+mux per-client 过滤+审批 claim**。**2026-08-27 M14 方案 v1.1 扩容收编**（全项目挂账盘点+逐项代码核验，M14 方案 §8）：M14 = TUI 线 V1-V5 + 债清偿线 C1-C5（协议收口/安全加固/宿主工程/web 对等/尾巴 ≈5 人天，全量 ≈10-10.5 人天）；**产品化定调（2026-08-24：奔产品非个人自用）——M14=配对+中继+E2EE+per-device token（orca 蓝本）+微信 ClawBot+企微+mux per-client 过滤+审批 claim**。**2026-08-27 M14 方案 v1.3 定稿待启动（用户审阅通过+D9 拍板，文件转 _待启动）**：M14 = **三线一体**——TUI 线 V1-V5（超屏防护+OutputViewer）+ 债清偿线 C1-C5（协议收口/安全加固/宿主工程/web 对等/尾巴）+ **产品化线 R1-R5（D9 拍板并入不立 M15，翻案零扫链；开工前另出细化方案）**；§8 全项目挂账收编（六处来源逐项代码核验）+四角色审阅修复 v1.2（P0×3：C1⑤ TUI 同帧消费证伪两侧改造同批/C1① 拆 a-b 事件重放缓冲移 R2/D9 论据改既成事实；**凭据条目化提前 C2①（D13）**/mux phantom subscriber C2⑧/C3⑤ 改 serve 补加载 skills/plugins（load 只在 REPL 路径——功能缺口）/claim=advisory+TTL（D12））；估时 C 线 8-9.5、V+C ≈13-15、R ≈13-15+（orca 四硬骨头：控制信道代次租约/director-cell 发现/信任根迁移/事件重放+微信 sender 白名单+审批短码绑定+E2EE v2 防重放），**三线合计 ≈26-30 人天**；真机门（G1/G3/G-IM/楔死销案）用户拍板先挂着。**同日 V+C 两线全部实施完成（十批代码提交链 9f94b99→9d95fd7+文档回填 7f8af5a，终态 1346+3skip+tsc 净+web 23 测+build 393KB；§3.3/§5 已回填实施记录）**——V 线：V1 viewport 基础层（computeBudget/useViewport/sectionBudget/foldLines 物理行折叠）/V2 溢出源截断四件（流式物理行化+ToolGroupView 展开上限+foldInputView 物理行感知+PanelShell rows 感知）/V3 OutputViewer（LineSource 三适配器：工具环形缓冲/任务日志 mtime 轮询/子代理 transcript；固定高滚动窗+follow+搜索）+TasksBar/V4 轮末即 commit（§3.3 查因：延迟 commit 是 M2 交互决策非时序修复，turn/completed 时 transcript 已终局无漏消息风险——方案一）/V5 allocateDynamic 总守卫（骨架+输入实占+条件段感知的预算分配；退化保护）+**真机验收探针 scripts/pty-overscreen-probe.cjs（18/40 行窗各 3 轮 80-delta 长流，全程无 ESC[3J 真兜底 0——硬指标达成）**；C 线：C2 安全八项（凭据三级 CredentialStore/Bearer-only/CSP/killServeByReg 身份核验/审批 claim+TTL/审计/SSE 背压/mux canAnswer）/C1 协议收口（item/completed 4KB 截断+item/read 全文/session/read 行分页/双 events 端点退役/浏览即装配收敛/mux sessionId 过滤）/C3 宿主工程（cli 拆三件 assembly·serveMain·cursor/P1-12 双开轮+滞留伴生竞态根治/asker 键控多宿主/commandRegistry 注入化/serve 补加载 skills·plugins）/C4 web 对等（interrupt 停止键/error·systemMsg·notice 渲染/askUser 表单//api/stats+StatsPanel；Streamdown 挂账）/C5 尾巴（restore 载入端口化/子代理压缩链 roles.summary 换笔/飞书 post 富文本；/projects 命令面挂账非尾巴级）。**M14 剩 R 产品化线（R1-R5）待另出细化方案**。
**2026-08-27 四角色审阅修复批（报告 docs/解析/2026-08-27_四角色M14-VC审阅_进行中.md c611b33→四批修复 c363cf5/253d835/cb2b570/694b9c8+文档批，1355+3skip）**：P0×3 全修（sessionId 路径遍历——冷路径白名单 isValidSessionId 活会话放行/OutputViewer 高度 reserve 6→10 任意终端必触发 3J 修/subagent 桥单槽跨项目串台——运行态四 getter 会话端口化）；P1 主体全修（会话身份贯穿：hook 权限门随发起会话路由+normalizeProjectPath 单点（services/pathnorm.ts）治 meta.cwd 三态与已活判定/C1⑤ web item/read 补漏+truncated 感知/stats·projects device 栅栏+topSessions 不外发/飞书 canAnswer:false+preview 围栏转义/killServeByReg 无 id 拒盲杀/查看器三连（getter 化补全竞态+cachedWrap 性能+truncated 落卡）/条件段感知 allocateDynamic+输入区实占 8/caret 字素口径/label clipWidth/探针补 /output 面板场景 O4 三段两连跑全过）；对标核验零虚报（1346 测试数精确吻合），薄弱面=汇总口径（十二批实为十批已正/V5 commit 数字示例失准 toolCap=2/stream=6 已正/C1⑤ web 缺口被完成措辞抹平已补）。**P2 余留**：claim 限长限频/SSE destroyed 标志/markdownToPost href scheme/HttpTransport 死客户端/401 速率限制/探针工具轮 mock 翻译（loop要求工具调用但未给出工具——归真机门排查）等随 R 线或按需档。R 线细化方案 v1.0 已立档（18867be 待审 D1-D7 拍板）。
**2026-08-28~29 Dogfood 升级飞轮（九批提交链 4575e92→c3fe527，方案 `详设/2026-08-28_后续-Dogfood实测修复方案_已完成.md` v1.9 全批次收官）**：三轮 dogfood 实录驱动——批1 CLI 校验/diff 中文名→批2a F-18 密钥根修→批2c 尾巴→批2d Notification hook 第七事件+BEL→**serve 事件驱动监看 ≈1.0×**（serve-watch.cjs 订阅 mux 自动应答审批，效率基线 3-5×→1.3×→1.0×）→批2b 审批卡交互五条（废除 CR 静默批准；四角色审阅+批2b-fix：draftPort 方案 B/F-10 超屏封顶/偏差②明示口径）→功能测试批（MCP 全链/skill 双触发/auto-memory/四面板/CC 对比锚点 4/4；F-24 skill junction 跳过+F-28 skillTool 注册表劈叉 makeSkillTool 工厂）→界面批七项（@补全/输入历史+Ctrl+R 项目级/Ctrl+E 单工具展开/**B2 零改动结论=Ink Static 即 scrollback**/acceptEdits 五档敏感硬门不降级/⏵⏵ 档位可视化/双击 Esc rewind；用户认可选型）→清账批 I（F-23 serve 命令分流 routed:'Command' 绝不落 LLM/F-26 /output 上下文/F-27 stop 优雅停机）→清账批 II（**批量 afterEach(cleanup) 30+ 文件——flake 家族 6 种缩到 2 种**；F-29 web warn 帧丢弃=假完成；F-09/F-06/F-11；dist 重建）。**监看方法论**：三面分流（日志=流水/历史=语义/TUI 快照=界面）+skill `~/.agents/skills/agent-dogfood-monitor/`（双模总则 serve 优先/§三b 驱动陷阱表）+ECode 自举（产品码派 serve 会话实施+监看方外部验收——onDraftChange 接线漏/测试卫生/E2E 断言虚报三漏均验收抓出；serve 续接截断轮成本极低）。**F 台账：P0 清零/P1 在账 F-16（验收冲突死磕不报告，提示词层下批）+F-07（三键 remember 接入待拍板）/P2 残余 flake 两只（checkpoint+approvalReason 深层时序）另案**；1510 总（2-3 红均 flake 家族单跑绿）+tsc 净+web 30/30+双探针过。**2026-09-03~04 全功能走查+四角色审阅两轮（报告 `docs/解析/2026-09-03_0831-0902全功能走查与审阅_已完成.md` + `docs/解析/2026-09-03_全项目功能走查与审阅二轮_已完成.md`）**：一轮 0831-0902 窗口 86 笔提交（输入体验/T 线附着/R 线配对 relay E2EE/活动流）产品码零回归实证、P1×10 全修（含 closeDeadTurn updater 副作用真 bug/设备快照服务端三处强制/401 限速/W-9 运行态对账+reattach keepSeq/baseline 竞态订阅泄漏/devices 合并落盘/web op settle/hostKeys 校验）+relay 安全修复已部署 nodetime.cn 复验 6/6；二轮全项目清单（斜杠 20/20 新探针 slash-sweep/工具集 16 真轮/服务面协议驱动）再提 P1×7（rewind 新建文件 absent 基线根治/审批零应答者如实反馈/MCP 批准门盲批外传+内容摘要/gate 中断盲区/软超时输家 reject/task_output abort 盲区/RewindPanel 确认失真）；**方法论沉淀：T 线后 mock 探针必须 ECODE_FORCE_EMBEDDED=1（自动附着让轮次跑进 daemon 打真端点假失败）**；并行会话同期合 34 笔（归属 meta 化 e268776 等）主干 4 红为其欠尾随手适配。
**2026-09-03 等待根治批（真机 loop-guard 误伤复盘：8 连发同参 task_output 轮询运行中全量测试撞 SIG_NUDGE）**：五家对标（CC/opencode/codex/openclaw/orca）结论「无人靠检测器开豁免——全靠让同参轮询不发生（单次长等待/完成推送/前台大超时/检测器做软）」→ 落地四件：① task_output wait_ms 上限 10s→300s（`TASK_OUTPUT_MAX_WAIT_MS` 定在 services/tasks.ts，对标 codex 5min/CC 10min）；② running 零输出响应带「已运行 Xs」（`TaskOutputResult.startedAt` 数据源——结果逐次变化，M13-P1 结果感知签名天然清零=合法等待豁免；终态保持静态=复读已结束任务的保护不丢）；③ bash 前台超时 30s 写死→输入参数 timeout_ms（默认 120s/上限 600s，对标 CC 同款数字；去掉循环层 `timeout_ms` 元数据转自管——顺带消除软超时不杀进程的孤儿面）；④ system.ts 指引+guard 提醒文案跟上新上限（「加大 wait_ms」从死路变活路）。回归锚：loop-guard 真工具测试（真机场景零触发+对照组 abort 不丢）+ bash 树杀测试迁输入参数形态。全量门现场 16 红**全数归属并行会话在途/存量**（pathspec stash 对照实证：摘掉本批 src 改动照样红；基线 14:58 跑 19 红同款）+tsc 净。
**2026-09-03 同名工具折叠批（用户拍板「TUI 相同工具能折叠也折叠——占位太大」：真机 4 连发 grep 平铺 8 行撞脸）**：病灶定位=动态区 TimelineView 无同名折叠（Static 连续纯工具轮本就合并组；每条 ToolLine 恒 2~3 行）→ 两处落地：① 动态区 `collapseSameToolRuns`（viewport.ts 纯函数，渲染与计价同源入口）：连续同名 run ≥3 且被折叠者全终态 → 「▲ name ×N 已折叠（Ctrl+T 全程）」摘要行+最新 1 条完整；text/thinking/异名打断（语义边界）；running 在 run 内不折叠（并行只读批活态可见）；edit/write 豁免（D15 diff 全量）；计价 tool-run=2 行、foldedSummary 计数按 count 还原不漏报。② Static `CompactToolGroup`（ToolGroupView）：全组同名非 edit/write → 组头「▢ bash ×7 · N 失败」+可见 2 条**单行/条**（digest+状态+preview 同行，六四分宽 clipWidth）+「还有 N 条（Ctrl+T 全程）」——N 条从 N×2 行收敛到 ≤5 行。验证：TDD 12 测（纯函数/动态渲染/计价协同/静态紧凑/豁免组）+真 pty 探针 `pty-toolrun-probe.cjs` 10/10（行收敛 ≤5+每行 ≤100 列无 wrap 溢出，CJK 计 2 列）+相邻 6 文件 74 测全绿+tsc 净；tests/tui 现场 7 红 stash 对照实证全归属并行会话在途（TuiAppHooks/commitThinking 等同款）。
**2026-09-03 两批三席审阅修复批（等待根治+同名折叠合并审：机制正确性/契约一致性/测试回归，P0×0）**：P1×5 全修——① bash timeout_ms 0/负数直通 setTimeout 立即杀树（「0=不限时」心智误传）→ schema `minimum:1`+运行时 `>0` 守卫回退默认（0/负直调测试锚）；② task_output 终态+空输出文案自称「仍在跑」（镜像面：把模型推向等死任务）→ 按 status 分文案（终态「任务已结束」）；③ 紧凑行 truncated 尾巴击穿单行（digest 顶满时 `⋯(NKB)` 追加在 preview 外）→ 尾巴+媒体标记宽度预扣进 preview 预算+分隔符实占 4→5 列修正（` ✓`2+` ▸ `3——宽度预算单测当场抓到差 1 列）；④ loop-guard 文案/system 指引硬编码 300000/120000/600000 → 常量插值（session/system import 常量，编译期定值不破 prompt cache）；⑤ wait_ms 与 timeout_ms 超上限契约风格相反（一静默钳一 AJV 拒）→ 统一 AJV maximum+运行时钳留防御纵深。补测 7 项：completed+空输出/0 负回退/waitMs 真子进程两测（提前返回+到期 running——机制本体此前零直接测试）/maxTools=1×紧凑组/startedAt 断言收紧（旧 t0-60s 恒真）/lines=2×run 折叠共存/紧凑行宽度预算（同源口径 cap=gutter+WIDTH.toolOutput）。P2 顺手：interface/loop timeout_ms 元数据注释口径更新（MCP 现存唯一用户/bash 自管/task 无超时）；多模态 blocks 紧凑行加 `[图片]` 标记。挂账：scripts 探针 clearTimeout（探针面并行会话树杀批在途，未动）；web 无同名折叠（挂 M 线）；紧凑组 todo 特化缺失（场景罕见）；「预计超 2 分钟」双处硬串（并行批锚定口径，单边改反致漂移）。8 文件 107 测绿+探针 10/10+tsc 净。
**2026-09-03 TUI /model 假切换根治+两席审阅修复批（真机：切 DeepSeek 无效——daemon iter 恒 glm/429 恒旧窗）**：根因=TUI /model 只 setConfig 本地 state 从未发 model/set 帧（协议帧现成仅 web 在用）→ 宿主（daemon）读自己 config 恒旧 current；且**产品 REPL embedded 同病**（装配 getConfig 持 makeDeps 原始 config，setConfig 不同步——帧 handler 就地改宿主 config 是两形态共同真同步面）。修复 b1cb267：onPick 补发 host.send model/set。审阅批：① P0 测试污染——embedded handler 就地写穿 TuiApp.test 模块级共享 config（「切 model 后 submit」测试直传模块级 config 绕过副本）→ makeDeps 每测 cloneConfig + 该测试改走 makeDeps 换 providerRegistry（图片粘贴 3 条连带红根治）；② P1 附着态快速连切 A→B HTTP 并发乱序静默停在 A → modelPickSeqRef 序号闸（落后回执丢弃）；③ P2 宿主拒绝回滚本地显示（防二次骗）+sticky notice 文案 remedy 前置；④ stale 注释更新（session.ts model/set handler 头「TUI 不经此」退役）。补测 ModelSwitch.test.tsx 3 条（fake-host：发帧双参断言/失败回滚+notice/乱序闸）——TUI 发帧面零测试缺口的首个行为锚。挂账：附着态 picker entries 改 config/get 投影（菜单只列宿主真能切项）；web 顶栏 model/set 同零测试。24+67 测绿+tsc 净。
> **永远使用中文与我对话。**
>
...
### 3.4 活文档同步（防漂移，权威清单见 docs/规范/2026-08-16_活文档清单与同步守则）

改代码时按下表「必改」同步文档——多数已被防漂移测试锁定（红=该查，修测试指向的文档）：

- 新增/改工具 → system.ts 工具指引（测试逼你）+ 工具 description。
- 新增 config 键 → ecode-config 手册字段表（测试逼你）+ CONFIG_TEMPLATE 注释（测试逼你）。
- 新增/改斜杠命令 → registry description + ecode-features 手册命令表（渲染派生+对账测试逼你）。
- 改快捷键/面板/多端能力 → ecode-features 手册对应节。
- 里程碑收尾 → 方案文档 status+实施记录、AGENTS 状态行、根 README「现状」节（清单 #18）、README 索引。
