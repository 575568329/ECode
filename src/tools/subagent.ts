...
export function createTaskTool(ctx: TaskToolContext): ToolDefinition {
  ...
  return {
    name: 'Task',
    description:
      '派遣子代理在独立上下文中完成一个可解耦的子任务，只回最终结论。\n' +
      '昂贵：每次调用 = 一次完整独立 LLM 会话，且你只拿到结论、看不到过程。\n' +
      '唯一适用：多个相互独立、可并行的子任务（如"同时分析前端和后端"）。\n' +
      '注意：「读多个文件」本身不是派 Task 的理由——ECode 有自动上下文压缩，主上下文不会撑爆，自己顺序 read_file 即可（哪怕 10+ 个文件、哪怕 Ctx 很低）。\n' +
      '不适用（直接自己用 read_file/grep/glob/bash）：读详设后读源文件、批量读取文件了解现状、按文档实施前先读相关文件、单文件/少量文件读取、线性调查、问答、单点修改、概览类任务（如"了解一下 X""查 X 做了没"）。\n' +
      '派前自己想清楚：子代理是全新上下文、看不到你的对话历史——别甩"根据你的发现去修 bug"这种它没上下文的活（它根本没有你的发现，必败）；派了就等结果或做不重叠的活，别自己又跑一遍同样的搜索/读取（白费）。',
    dangerous: true,
    parameters: {
      type: 'object',
      properties: {
