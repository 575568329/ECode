...
- [debugging.md](./debugging.md) — 踩坑：#001 env 不覆盖 / #002 env -i 丢 TMPDIR / #003 fast-glob ESM / #004 LLM 知识失真 / #005 上下文超限恢复 / #006 GLM 端点 / #007 Session ID 碰撞→UUID / #008 全功能日志覆盖 / #009 `<Static>` append-only→换 key 重 mount / #010 文档 file:line 漂移 / **#011 Windows CRLF 陷阱** / **#012 Windows bash 三连坑（阻塞+find.exe+GBK）** / **#013 ink `exitOnCtrlC` 默认 true → Ctrl+C 逻辑成死代码 + testing 盲区** / **#014 中断识别用 signal.aborted 不用 instanceof（SDK 包装的 abort 错误漏判→显示 ✗ Request was aborted）** / **#015 M5 三源联网研究推翻 7 处早先假设（不瞎想实证）** / **#016 `npm test` 是 watch 模式（全量一次性用 `npx vitest run`）** / **#017 MCP 启用→内置工具被喂丢→LLM 瞎编参数死循环（先查工具集别先怪模型 + 治本>prompt 补丁 + Plan C 按 toolName+isError 键）** / **#018 模型名查询大小写不敏感（用户 config 大写 GLM-5.2 vs 小写查询→getContextWindow 兜底 128K）** / **#019 REPL 退出 MCP 子进程残留（process.exit 跳过 React 异步 cleanup）→ shutdown 统一退出 + 单例 + fast-path** / **#020 MCP 工具结果单行 JSON 转义串刷屏（foldContent 截断失效）→ prettifyCompactJson 美化**
