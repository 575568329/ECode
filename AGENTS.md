...
> **当前状态**：M1（心脏）+ M2（TUI 最小 Static）+ M3（工具集+ConfirmPrompt+bash 安全+.ecodeignore）+ M4（HistoryStore/向导/双协议 Provider）+ M5（上下文压缩：投影分离+分批 map-reduce 800k→200k 58s+cache 四维成本）+ M6（Skill 双触发面+MCP 按需加载+面板+内置 ecode-config 手册）+ M7（HookRunner 六事件+Plugin 安装链+PluginPanel）+ **M8（交互与上下文智能：ECODE.md 两级注入+截断可配/ask_user 选项框/auto-memory/WebFetch+SSRF/分段化/债清账+方案外交付：告警中心/面板+ /doctor 自检）全部完成**（2026-08-16 实施，852/852+两轮审阅修复；流式 markdown 彻底放弃 M8-D12）。后续 **M9 = 安全网与质量闭环**（**待启动，下一个**）（hooks 接线修复 P0/checkpoint+/rewind/lint-test 回喂/git 轻量/沙箱四档默认关（含 full-access 总放行+用户可配 blockedCommands）/**权限系统首步 Hook(owner)**/LSP 占位，`2026-08-15_后续-M9实施方案_待启动.md`，未排期裁剪见其附录 D 观察区）→ **M10 = 感知扩展与配置可视化**（多模态输入 ImageBlock：read_file 路径引用+双协议翻译 / WebSearch：智谱 API 首选同 key 体系 / /config 应用内面板：三页签+jsonc 非破坏修改+$EDITOR 逃生口，`2026-08-16_后续-M10实施方案_待启动.md`）。
> **永远使用中文与我对话。**
>
