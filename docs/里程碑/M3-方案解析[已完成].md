...
> **关联**: [M3-实施方案](./M3-实施方案[已完成].md)(怎么做:接口/调用链/文件清单)、[00-开发规划](../总纲/00-开发规划[进行中].md) M3 章节、[M2-方案解析 §4.3](./M2-方案解析[已完成].md)(孤儿配对坑的事实依据)
>
...
> 这一章是 M3 最难、最高风险的部分。实施方案 [§五](./M3-实施方案[已完成].md) 讲了"怎么做",这里讲"为什么这么做"。
...
- 写盘是**整文件覆盖**(同 session 文件,messages 累积),不是 JSONL 追加——一个文件 = 一份可直接 `loadSession` 的完整状态(为什么不用 JSONL 事件溯源见 [实施方案 §6.2](./M3-实施方案[已完成].md):YAGNI,ECode 不做分支 / fork)
...
**理由**:session 是**附加能力**(让会话可续接),失败顶多"本次不可 --continue";若因此杀 loop,跑到第 20 轮修 bug 时磁盘满 = 前 19 轮全白干 + 任务没完成。完整规则(持续重试 / 写盘重试一次 / 可见度)与证据表见 [实施方案 §6.6](./M3-实施方案[已完成].md)。
...
有争议,所以列在 [实施方案第十二章待审阅](./M3-实施方案[已完成].md)。支持纳入的理由:M3 压缩要在 messages 上操作,`providerOptions` 侧信道能挂"可压缩"标记;P5 的 usage 细化、并行工具标记都要挂在升级后的类型上。不升级,M3/M4 反复修补核心类型,更痛。反对的理由:P1 是重构,有回归风险,会让 M3 变大。**这是范围决策,需用户拍板**。
...
- [00-开发规划](../总纲/00-开发规划[进行中].md) M3 章节与认知盲区 P0-3(截断)/P1-1(项目记忆)
- [借鉴Vercel-AI-SDK对比报告](../调研/借鉴Vercel-AI-SDK对比报告[已完成].md) §四⑥⑦(providerOptions/声明式工具)、§四④(retry)
- [M2-方案解析 §4.3](./M2-方案解析[已完成].md) 孤儿配对坑的事实依据
...
**关联文档**: `./M3-实施方案[已完成].md`(怎么做)、`../总纲/00-开发规划[进行中].md`、`./M2-方案解析[已完成].md`、`../调研/借鉴Vercel-AI-SDK对比报告[已完成].md`
