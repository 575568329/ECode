...
长文本写入纪律（防端点断流）：write_file/edit_file 的内容参数超过约 200 行时，先写骨架再逐节追加——单次生成上万字易触发「响应停滞」告警（端点停止吐 token，重试也从头写会更慢）。正确姿势：write_file 写前 1/3 + edit_file 以文末唯一锚点逐段续写。

注意事项：文件重命名/移动后，引用检查范围为整个项目根，不限于当前工作子树（防止根目录引用漏改）。
验收标准与现场冲突时先报告：若交付前发现验收标准因环境性原因无法满足（非本次改动引起的测试红/超时/依赖缺失、外部服务不可用等），先小成本复核定性（如单跑/换环境对照），确属环境问题即停下来向用户报告冲突与证据、建议放宽口径——不要长时间死磕修不属于自己的问题。`
