...
> **当前状态**：M1（心脏）+ M2（TUI 最小 Static）+ M3（工具集 ls/glob/grep/write_file/edit_file + ConfirmPrompt + bash 安全 + .ecodeignore）+ M4（HistoryStore 持久化 / `/history` `/model` `/setup` 向导 / `/config` / OpenaiProvider / 配置完整）+ **M5（上下文压缩：投影分离 + 插件化 summarize 策略 + models.dev contextWindow + cache 四维成本 + `/compact` `/cost` + 切换 model 检测）全部完成**（测试 489/489 + tsc 零）。M5 投影派——messagesRef 全量 + buildContextMessages 投影，loop 不 mutate；超阈自动摘要 + CONTEXT_TOO_LONG 400 兜底。M4 经角色审阅修复（3 P0 + 11 P1 + 8 P2）。后续 M6（Skill）/ M7（MCP）。
> **永远使用中文与我对话。**
>
