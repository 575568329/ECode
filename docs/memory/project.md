...
| M6 收尾里程碑 | M1-M5 收尾 + npm 发布 + Skills(13)/路由(22)/多渠道(23) | 🚧 **M6 实施中 2026-08-09**（重组为收尾里程碑：吸收 M1-M5 收尾项 + npm 适配 + 原范围；Repo Map 拆出后续扩展功能；一口气全做→v0.2.0；D2=服务化+Web 前端），**阶段 A ✅ 完成 2026-08-09**（文档刷新纠偏 + homedir 收口实证：数据目录已收口 / index.ts 指令加载提取 buildSystemInstructions helper + 路由 alias 解析器纯函数 src/router/，6 单测，851 全绿）；**阶段 B ✅ 完成 2026-08-09**（npm 发布适配：files 白名单/prepublishOnly/repository/LICENSE/CHANGELOG/README 全补，npm pack 无敏感泄漏 253.6kB；真实 GLM 端到端连通验证 stopReason=stop）；**阶段 C ✅ 完成 2026-08-09**：①cost 精确化（支点17，28ee89a：ModelConfig.cost 单价驱动 + computeCost 纯函数 + Anthropic/OpenAI inputTokens 统一为「总输入含 cache」 + status-bar $金额 + /cost 拆分 cache·reasoning，跨层复用 ECodeUsage）；②Hooks Stop 事件流（5bc4aba：HookGate.emit 聚合层 + agent 四注入点 SessionStart/UserPromptSubmit/Stop打回/SessionEnd，Stop deny→push user 续跑，仅 done 触发防死循环）；**阶段 D ✅ 全链路完成 2026-08-09**（本轮自主推进 S2-S6+R1-R4，待用户审阅详设）：阶段1 Skills 手写**完整**（1248641：types/loader/catalog/matcher 对齐子代理支点9 模式 + system 注入 skillsCatalog 懒加载 + /skill 命令，17 单测）+ 阶段3 路由**核心+接线完成**（c219623 rules 纯函数 8 单测 + router/config.ts routing 块读取 7 单测 + **三触发点接线**：global（runAgentStream 入口 opts.model 优先>路由>default）/ compress（阈值用主模型 contextWindow，summarize 走 compress 场景模型；⚠️ 无规则须回退主模型 resolvedModel 而非全局默认，否则 mock model 测试误触发 createProvider 要 key） / subagent（ctx.routingConfig 可选，测试不传回退 ctx.model；2 正向单测））+ 阶段2 security-scan 核心（7b84444：scanProposal 危险模式兜底 D7 = rm -rf/密钥/外发/fork/chmod 777，10 单测）；**本轮新增（技能生成 S2-S6 + 模型路由 R1-R4）**：S2 记录层（redact 脱敏 + recorder FIFO/去重 + UserPromptSubmit 接入）/ S3 scanSkillContent 三段式（rm-rf 锚定 + 中文注入 + base64 外泄）/ S4 归纳层（frontmatter 手 split round-trip 零依赖 + generator Ratchet LLM 批归 + /skill-gen 命令接线）/ S5 审批层（proposal 状态机 accept/reject/edit/promote + skill-dialog 交互审批 picker+详情 + /skill 重构 picker/accept/reject/promote 子命令）/ S6 promote 用户级（resolveDataDir/skills 同名跳过 + --force）/ R1-R2 assessComplexity 纯函数 + complexity 档位配置 / R3 resolveModelForSubagent+subagent 接入+metadata 6 层透传（ToolResult→AgentEvent→DisplayMessage→ToolDone→BlockTool via-line）/ R4 tool-panel Task 气泡渲染 metadata；**1053 全绿**（97 文件，排除 flaky mcp-client）；中断 UX（Ctrl+C 分情况：A=LLM 未回应→撤回 user 气泡+回填输入框+不显示中断 / B=已回应→显示中断+保留 partial；controller 跟踪 turnResponded，controlled prop draftVersion 替代 forwardRef 因 ink7 破坏 useInput）。⚠️ **D3 两处架构限制**：①skill 场景无独立落点（/skill 当前 api.submit(body) 注入主对话不独立 session，SkillDefinition.model 声明未生效，待 skill 独立化）；②subagent provider 继承主代理（ctx.provider），跨 provider（cheap=deepseek 而主=zhipuai）不生效，仅同 provider model 切换，跨 provider 需 provider 继承重构（子代理按模型独立 createProvider）；③status-bar 路由来源标签不做（主对话恒 global 无信息量，compress/subagent 瞬态不持续显示，已显模型名足够）。✅ **D2 已落地**：generator+proposal 接线改走 `/skill-gen` 命令式触发（非 Stop hook——Stop hook 是 shell command 无法调 TS 函数，详设 §3 已论证），归纳→提案→/skill 审批全链路通；subagent 跨 provider 路由 R3 落地（resolveModelForSubagent 按 persona>complexity>rule>default 解析，跨 provider createProvider 失败降级 ctx.provider 非 isError）。⚠️ **遗留**：①✅ edit 已消解（经评审无独立需求：提案/已安装 skill 均普通 .md，用户自行编辑后 /skill 重载即可；已删 editProposal 函数+[e] 键，审批精简为 accept/promote/reject 三选一）②accepted/promote skill 落 flat `<name>.md`（loader 读顶层 *.md，非设计 §8 的 `<name>/SKILL.md` 子目录——以 loader 为准，维持现状）③✅ maxPending=50 FIFO + 跨批同名去重已实现（generator Map 去重 + enforceMaxPending 按 mtime 升序删最老 + GenerateResult.pendingAfter + /skill-gen 达上限 UI 提示）④✅ 跨 provider 降级已补 runtime-logger warning（logWarning，排查有迹；仍 §9.3 优雅降级非 isError）。⏸ 待续：C3 真机冒烟 4 项（Ctrl+O/bash/消息不重复/多行 Shift+Enter）+ 阶段 E（多渠道服务化+Web，🔴 前置已核实：CLAUDE.md §4.2 React 全栈是残留模板——react ^19 仅 ink peer dep，无 Zustand/Tailwind/Vite，Web 前端需从零引入全栈 + Vite §9.3 原生二进制红线）+ 阶段 F（P2/P3 打磨） |
