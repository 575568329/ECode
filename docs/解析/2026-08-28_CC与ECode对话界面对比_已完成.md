...
> 标注约定：【官方】= code.claude.com/docs 明确记载；【实测】= Claude Code 本机驱动会话与通用实测行为（官方文档未描述）。
...
7. **acceptEdits 档**（3.4）——「自动接受文件编辑、bash 仍确认」是 CC 高频档位；ECode 沙箱四档里 default→workspace-write 之间缺这个粒度（workspace-write 是否含 bash 免确认需查 services/sandbox.ts 语义）。
