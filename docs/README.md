...
| 后续 M6 实施方案 | [详设/2026-08-14_后续-M6实施方案_待审核.md](./详设/2026-08-14_后续-M6实施方案_待审核.md) | **v5（补二级菜单面板 TUI）：M6 = Skill + MCP 两子系统，Plugin 推迟 M7**——Skill 三层渐进式加载 + **双触发面**（LLM SkillTool + `/skill args` 手动 `$ARGUMENTS` 传参，双布尔独立开关）+ token 预算 + 蒸馏 L2（创建+升级合并协议）；MCP 官方 SDK + **按需加载**（lazy 默认+metadata cache+六态状态机+failed 60s 过期退避+空闲卸载）+ 默认确认 + 首用批准；**面板 TUI（v5）**：PanelShell 基础设施（复用 overlay 挂载+viewState 多级导航+Esc 逐级回退+窗口滚动+即时搜索）+ SkillPanel（分组列表+Enter 回填选用）+ McpPanel（列表→详情→工具三级+重连/断开乐观更新+failed 错误展开）+ PluginPanel（M7：三页签市场浏览/搜索/详情/安装）+ 统一键位规范；v4 双触发面/按需加载/升级合并 + v3 审阅修订全保留；全文第一人称，参考来源集中附录 B；心脏零改动；M6-D24 不采纳 ADR-026 |
