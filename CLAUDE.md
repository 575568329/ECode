...
| `docs/00-开发规划.md` | 总纲（定位 / 原则 / 里程碑 / 盲区清单 / 架构） |
| `docs/M1-*.md` | M1 阶段三文档：技术选型与理由 / 实施方案 / 方案解析 |
...
- **里程碑文档分级**（设计文档）：每个里程碑 MX 固定三文档，扁平放 `docs/` 下，文件名带 `MX-` 前缀（单独打开也能看出归属）：
  - `MX-技术选型与理由.md` — 该阶段技术选型与取舍
  - `MX-实施方案.md` — 实施步骤 + 验收标准
  - `MX-方案解析.md` — 原理 / 协议 / 设计答疑（合并该阶段所有原理性资料）
  - 做到某个里程碑时再建对应三文档（YAGNI，不预先占位）。
- **一次性 / 临时文档**命名：`YYYYMMDDHHMMSS_文档标题`（如日志、临时笔记）。日常原理答疑优先并入对应里程碑的「方案解析」，**不另建 notes**。
