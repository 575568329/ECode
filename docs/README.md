...
| 后续 M6 实施方案 | [详设/2026-08-14_后续-M6实施方案_待审核.md](./详设/2026-08-14_后续-M6实施方案_待审核.md) | **v3（审阅后拍板）：M6 = Skill + MCP 两子系统，Plugin 推迟 M7**（第三部分保留为 M7 设计储备）——Skill 三层渐进式加载 + token 预算 + 蒸馏最小闭环；MCP 官方 SDK + stdio&HTTP + mcp__server__tool + 默认确认 + 首用批准；v3 修订：MCP 工具 token 预算计入压缩估算 / register 跳过外部 schema 预编译 / 盲确认修复 / M5 债处置（#3 DRY→S-P2）；心脏零改动经核实成立；M6-D24 不采纳 ADR-026 AppContext |
