...
describe('executeEditFile', () => {
  ...
  it('oldText 唯一匹配时替换成功', () => {
    writeFileSync(tmpFile, 'hello world', 'utf-8');
    const result = executeEditFile({ path: tmpFile, oldText: 'world', newText: 'ECode' });
  ...
  it('oldText 多次匹配时报错并提示补充上下文', () => {
    writeFileSync(tmpFile, 'dup\ndup', 'utf-8');
    const result = executeEditFile({ path: tmpFile, oldText: 'dup', newText: 'x' });
    expect(result.isError).toBe(true);
  ...
  it('空 oldText 不替换（视为未找到）', () => {
    writeFileSync(tmpFile, 'abc', 'utf-8');
    const result = executeEditFile({ path: tmpFile, oldText: '', newText: 'x' });
    expect(result.isError).toBe(true);
