...
主模型跑常规轮，高级 reviewer 模型「定时兜底 + 异常信号提前触发」出纠偏卡注入（只审查不接管——不改变任务执行流）。异常信号走同步 gate：暂停在下一工具批前等审查结果（超时放行），纠偏卡赶在下一个动作之前到达。
...
| timeoutMs | number | 60000 | 信号 gate 超时 ms：信号触发后下一工具批前同步等审查，超时放行继续（结果稍后到达） |
