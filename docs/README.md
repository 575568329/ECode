...
| 真机诊断修复方案 | [详设/2026-09-02_后续-真机诊断修复方案_已完成.md](./详设/2026-09-02_后续-真机诊断修复方案_已完成.md) | **已完成（v1.1，三批提交 0b93e29/4451f7c/5e8d2fd/89780f8+0854c9b，文末实施记录）**：搜题平台三连症全根因闭环——P0-A Ctrl+C 立即停（signal 传 create() 第二参+逐 chunk 检查+哨兵）、P0-B 停滞看门狗（STREAM_STALL 转译/零产出重试约束/streamStallMs）、P1-A 客户端内存四层（**计划外第 4 层：NODE_ENV 缺设→react-reconciler 恒 dev 构建每渲染滞留（真机 1.3G 真泄漏本体）**+增量折叠+thinking 合帧+memo 门+RSS 专项探针）、P1-B pinnedKeys 恒留；真机门待用户 dist 重建实测 |
| 移动连接/二维码/skill/MCP 专项测试 + MCP2 调研 | [解析/2026-09-04_移动连接与二维码与skill与MCP专项测试_MCP2调研_已完成.md](./解析/2026-09-04_移动连接与二维码与skill与MCP专项测试_MCP2调研_已完成.md) | **稳定性实测 8/8**（公网 relay E2EE：延迟 108-203ms/重连风暴 10/10/沉默 35s 自愈/双设备并发路由正确——fresh 4409 只管控制腿，数据腿 maxLegs=8 共存是产品语义）+ **QR 解码回读 9/9**（jsqr 等价相机扫描，终端 QR 与深链 603B 逐字节一致）；skill 手动触发+敏感门不绕过行为实证；MCP 全链真机通（stdio spawn/工具调用/审批/退避恢复）；MCP2 调研=SDK 1.30 已在新规范上，不建议协议升级，resources/prompts 能力面采纳待拍板 |
| 全项目功能走查与审阅二轮 | [解析/2026-09-03_全项目功能走查与审阅二轮_已完成.md](./解析/2026-09-03_全项目功能走查与审阅二轮_已完成.md) | **全功能清单四层走查+四角色审阅二轮+P1×7/P2×12**：斜杠命令 20/20（新探针 slash-sweep：增量断言+沙盒接线）/工具集 16 真轮全过/服务面（ECODE.md 注入/沙箱真拦真放/多模态 path 形态/ask_user 全链/hooks 正确 schema/压缩）；rewind 新建文件回退根治（absent 基线）+审批零应答者如实反馈；二轮审阅 P1×7（gate 中断盲区/软超时 reject/task_output 5min 盲区/absent 三席收敛静默删/RewindPanel 确认失真/MCP 批准门盲批外传+内容摘要/测试席三锁）；挂账十二项随下批 |
| 0831-0902 全功能走查与审阅 | [解析/2026-09-03_0831-0902全功能走查与审阅_已完成.md](./解析/2026-09-03_0831-0902全功能走查与审阅_已完成.md) | **四层走查+四角色审阅+P1 全修**：86 笔提交面（输入体验/T 线附着/R 线配对 relay/活动流）产品码零回归——9 红测试+11 处探针滞后全部是测试资产未跟上拍板；系统性根因=T 线自动附着打破探针环境假设（mock 探针必须 ECODE_FORCE_EMBEDDED=1）；实测基线 Ctrl+C 109ms/relay 公网 6/6/timeline 帧 thinking 31 帧；四角色审阅 P1×10（closeDeadTurn 提示静默丢失真 bug/设备快照服务端强制三处/401 限速/W-9 运行态对账+keepSeq/baseline 竞态/devices 合并落盘/web op settle/hostKeys 校验）+P2 快赢 14 项全修，relay 侧已部署 nodetime.cn 复验 6/6；挂账九项随下批 |
