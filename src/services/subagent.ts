...
何时用：大范围搜索/多文件调研归纳/互相独立的并行子任务/不需要过程细节只要结论的工作/多视角审阅与对照核验（每席一个子代理独立出结论，调用者汇总）/需要独立第二意见时。
何时不用（直接自己做更快更准）：读指定文件、找特定定义、2-3 个文件内的小改动、需要当前对话上下文的任务。

并行委派：把多个独立的 task 调用放在同一轮里一并发出——同轮多个调用即并行执行（并行池上限 8 个同时跑），不要逐个派完等结果再派下一个。用户明确要求并行/多角色时，必须同轮发出全部 task 调用。委派后不要自己重复做已委派的工作——等结论，或做与子任务不重叠的其他事；子代理的结论由你汇总转述给用户（结果本身用户看不到细节）。
...
示例（多角色审阅）：用户要求审阅提交 X 时（小 diff 如 ≤3 文件可直接自己审），同一轮发出多个 task，每席一个——
① description「架构席审阅」prompt「你是架构审阅席：审提交 X 的 diff（git show X），只评分层设计/模块边界/协议一致性，输出 P0/P1/P2 清单（每条：问题+file:line 证据+建议），不改代码」；
② description「安全席审阅」prompt「（按①的句式展开为完整自包含任务书，视角换成注入面/越权/护栏——勿把此省略括号照抄进 prompt）」；③ description「测试席审阅」prompt「（同上展开，视角换成断言强度/覆盖缺口/实跑测试）」。全部返回后汇总为一份审阅报告。
席位需要跑 git/shell 命令的（如 git show）用默认 general 型——explore 型没有 bash 工具。

注意：单次调用阻塞至子代理返回结论（同轮多个 task 调用不受影响，仍并行），无 task_id/task_output 轮询——那是 bash run_in_background 后台命令的机制。`,
    input_schema: {
      type: 'object',
      properties: {
