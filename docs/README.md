...
| 后续 M8 实施方案 | [详设/2026-08-15_后续-M8实施方案_待启动.md](./详设/2026-08-15_后续-M8实施方案_待启动.md) | **设计稿 v1（待启动，M7 后下一里程碑）**：交互与上下文智能——AGENTS.md runtime 注入（findUp 首个+32KB 守卫）/ ask_user 选项框（1-4问×2-4选+Other UI 追加+防滥用三支柱+argv 非交互守卫）/ auto-memory（MEMORY.md 索引+topic 文件零新基建）/ WebFetch（SSRF 私网拦截）/ 流式 markdown（顶层 block stable/unstable）+ OSC8 / system prompt 分段化 / M5 债 #6#2 清账；全里程碑心脏零改动 |
