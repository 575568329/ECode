...
- **沙箱**（**默认关，用户主动开**——default 档=现状确认弹窗，2026-08-16 拍板）：`/sandbox` 三档（default / read-only / workspace-write）；write_file/edit_file 越界拦截；read-only 下副作用工具整体拒绝
...
```
/sandbox 切换档位（StatusBar 常驻显示当前档），三档各拦什么：
...
| P4 | 沙箱三档 + /sandbox + StatusBar 标识（default 档=现状=关）| 无 | 零（工具 execute 前置校验）|
