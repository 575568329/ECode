...
| 后续 Dogfood 实测修复方案 | [详设/2026-08-28_后续-Dogfood实测修复方案_待审核.md](./详设/2026-08-28_后续-Dogfood实测修复方案_待审核.md) | **v1.6 批2b+审阅修复批全部完成**：五批提交链（批1 4575e92 CLI 校验+diff 中文名→批2a F-18 根修+探针六修→批2c be3ccd7 F-18 尾巴五项→批2d 15b9957+9c87945 Notification hook 第七事件+BEL→serve 事件驱动监看 ≈1.0×→批2b 9c12ab1 审批卡交互五条+F-10/F-13/F-21/F-22 废除 CR 静默批准）；F-08 半活楔死正式销案（7e01439）；批2b 四角色审阅 §13.8（P0×5）+批2b-fix §13.8a（F-10 展开超屏封顶/草稿状态机方案 B draftPort/②偏差四面裁决=明示口径；1449 总+探针双过）；效率基线 pty 手动 3-5×→full-access 1.3×→serve ≈1.0×+skill 沉淀 ~/.agents/skills/agent-dogfood-monitor/ |
