...
| 后续 M10 实施方案 | [详设/2026-08-16_后续-M10实施方案_待启动.md](./详设/2026-08-16_后续-M10实施方案_待启动.md) | **设计稿 v1.3（待启动，排在 M9 后；三次修订：v1.1 调研 / v1.2 四家多模态一手源码调研 / v1.3 四家子进程一手源码调研）**：感知扩展与配置可视化——多模态输入 ImageBlock（**ToolResult.blocks 扩展**+magic bytes 判定+双协议翻译含 **OpenAI 侧 image 转移**/5MB+8000px 守卫/压缩历史不存 base64/非 V 模型守卫）+**PDF document block 同管道（v1.2：三家 CLI 共识+GLM 侧验证关卡+不做页渲染兜底）**+**图片粘贴 P2 可选（v1.2：Win PowerShell/macOS osascript）** / WebSearch（**智谱 HTTP 规格已核实**：同 key 体系，engine 默认 std ¥0.01/次）/ /config 应用内面板（三页签+jsonc modify 非破坏+$EDITOR 逃生口）/ **后台子进程任务（v1.3 §4：bash run_in_background+TaskRegistry+task_output 增量读/task_stop+统一杀树 Unix 进程组/Win taskkill+SIGTERM 梯度；顺带发现并立项修复 bash 单点 kill 孙进程泄漏现存缺陷；否决 PTY 会话/Job Object/shell snapshot）** / **§9 滚动待办与排期（M10 内六项/M11 TUI 债候选十三项/M12+ 按需/工程债随手清单）** |
