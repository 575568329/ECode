...
**这是什么项目**：用 TS + Node.js 从零**手写** AI coding agent（**开源产品**，对标 Claude Code）。单包 CLI，核心是手写的 agent loop（不用 LangGraph 等框架）。交付优先：测试 / 错误恢复 / 权限 / 文档都是必须项。完整规划见 `docs/总纲/00-开发规划.md`。如目标文档不明，先扫 `docs/README.md` 找分类。

**会话开始按顺序读**：
1. 本文件 **§九**（红线避坑）——必须遵守。
2. `docs/memory/MEMORY.md`——项目状态索引（进度/决策/踩坑），**跨设备同步靠它**。
3. 按需读 `docs/memory/` 下对应文件 + `docs/` 设计文档（动手前读相关方案，分类见 `docs/README.md`）。
...
| `docs/README.md` | docs/ 目录索引与分类导航 |
| `docs/总纲/` | 项目定位、规划、审查 |
| `docs/里程碑/` | 里程碑三件套（M1~M4） |
| `docs/功能方案/` | 功能设计方案 |
| `docs/详设/` | 子功能详细设计 |
| `docs/调研/` | 调研对比报告 |
| `docs/诊断/` | 问题诊断 |
...
- **里程碑文档分级**（设计文档）：每个里程碑 MX 固定三文档，放 `docs/里程碑/` 下，文件名带 `MX-` 前缀（单独打开也能看出归属）：
  - `MX-技术选型与理由.md` — 该阶段技术选型与取舍
  - `MX-实施方案.md` — 实施步骤 + 验收标准
  - `MX-方案解析.md` — 原理 / 协议 / 设计答疑（合并该阶段所有原理性资料）
  - 做到某个里程碑时再建对应三文档（YAGNI，不预先占位）。
- **一次性 / 临时文档**命名：`YYYYMMDDHHMMSS_文档标题-分类后缀.md`，按分类进 `docs/详设/`、`docs/调研/`、`docs/诊断/`、`docs/测试/` 等子目录。日常原理答疑优先并入对应里程碑的「方案解析」，**不另建 notes**。
- 设计文档更新时同步改引用（重命名 / 删除文件要全量 grep 改链接，避免断链）。
- **新增 `docs/` 下任何 `.md` 文件时，必须同步在 `docs/README.md` 对应分类追加一行。** 这是 CCode 教训——README 与目录脱节即失效。
...
本项目前端已于 2026-06-05 从 Vue 3 重构为 React。
