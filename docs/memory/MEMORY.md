...
- **里程碑**：M3 P1-P4 完成（含 L3 forceCompact 已接线 2026-08-07）；M3.5 交互式 CLI 进行中（REPL/斜杠/折叠/pager/会话/键位分工/多行输入已落地）；**M4 权限系统全 5 阶段完成 2026-08-08**（b60c36b：path-guard + check() 三档 + 修 🔴-2 + bash 命令分级 + settings-loader + CLI 模式切换 + deny 接线 + UI 三态审批/doom-loop/危险高亮，683 单测全绿）；**M5 三支点全部完成 2026-08-09**（阶段0 地基 + 阶段1 子代理(76abd74) + 阶段2 Hooks Pre/Post 引擎(e7148a4) + 阶段3 MCP 全链路（R1 SDK + R2 client RCE白名单 + R3 斜杠注册式重构 + R4 adapter + R5 loader接线 + R6 /mcp UI + R7 prompts→斜杠命令 + Hooks settings.json 配置加载），819 单测全绿，tsc clean）；**M6 设计完成 2026-08-09**（三文档+审阅改定：Skills+模型路由+多渠道，6 致命+8 改进全修，D1-D7 待审阅，代码未开始）
