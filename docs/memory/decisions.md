...
## 决策 #007：不向 LLM 暴露剩余迭代预算（推翻 #021「让 LLM 感知预算」教训）

**日期**：2026-08-10
**状态**：✅ 已决策（bug 修复：80% 告警诱导假性完成，用户确认删除）
**影响范围**：agent loop 收尾行为——`src/agent.ts`（删 80% 告警 + 达上限诚实总结）、`src/providers/config.ts`（默认 40）、`src/tools/registry.ts`（todo 防假性完成）

### 被推翻的决策（#021 教训）

[debugging.md #021](./debugging.md) 三件套第 2 点原教训：「agent loop 必须有迭代预算管理——不只是上限，**还要让 LLM 感知到约束**」。据此 ddbe708 在迭代达 80% 时注入软告警「你已使用 N/M 轮…请尽快收尾」。

### 推翻理由（实证 + 调研）

1. **实证**：用户发现 agent「todo 没执行完就收尾」。runtime-log 显示告警触发当轮，LLM 立即回「预算告警触发，立即收尾」并放弃未完成 todo——**告警直接诱导假性完成**。
2. **opencode 源码**：`steps: Infinity` 默认，全程不向 LLM 暴露预算；硬上限用 assistant-prefill `MAX_STEPS_PROMPT` + `tools:[]` + `toolChoice:"none"` 强制总结。
3. **openclaw 源码**：模型自决停止，全程不暴露预算；硬上限即报错。
4. **共识**：两家成熟产品都不做软告警——向 LLM 暴露剩余预算会诱导「应付收尾」而非「完成任务」。

### 新方案

- **预算对 LLM 全程隐藏**（无 80% 告警）。
- **达上限强制诚实总结**（对齐 opencode `MAX_STEPS_PROMPT`，用 user 消息注入适配 ECode 双协议）：禁工具 + 总结指令（已完成/未完成/后续）+ 追加一轮纯文本。
- **默认 25→40**（长任务余量）。
- **todo_write 描述硬约束防假性完成**：完成须实际工作+验证，禁止基于意图标 completed。

### 关联

- 踩坑详解：[debugging.md #021 二次修正](./debugging.md)（含 runtime-log 实证 + 三家对照表）
- 提交：`5fa6691`
