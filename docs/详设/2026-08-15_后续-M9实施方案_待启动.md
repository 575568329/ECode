...
```
【事中：每次改文件前先拍快照】
LLM 要执行 write_file / edit_file / bash（副作用工具）
   → 弹确认，用户同意
   → ★ 快照：把「即将被改的文件」当前内容复制到
       ~/.ecode/checkpoints/<会话ID>/<序号>/
       （附 meta：什么时间 / 哪个工具 / 改哪些文件）
   → 才真正执行修改

【事后：用户反悔了】
用户敲 /rewind → 面板列出快照点（时间 + 当次改了什么）
   → 选一个点（界面再确认一次，列出将还原的文件）
   → 文件还原：从选中点往回，按时间倒序把文件内容放回去
      （后改的先还原，一层层退回去）
   → 对话回退：追加一行 rewind 标记
      （★历史文件一个字不删——只是标记之后的对话不再喂给 LLM，
...
一轮对话中 LLM 改了文件（write_file / edit_file 成功）
   │ 等这轮工具都执行完（轮末，同轮改多个文件只跑一次）
   ▼
自动跑 lint / test（命令自动从 package.json 的 scripts 里找）
   ├─ 全绿 → 静默（不打扰任何人）
   └─ 有失败 → 失败输出作为提示信息喂回给 LLM
        │ LLM 看到具体报错 → 自己动手修
        ▼
      修好了？ → 回到绿灯 ✓
      连续 2 次修不好 → 停止自动跑，提醒用户接手（防死循环烧钱）
```
...
```
【自动提交】（config 开启 autoCommit 后，默认关）
LLM 改完文件 + lint/test 跑完（轮末）
   → git add 改动的文件 → git commit
      message：由模型一句话生成（失败就用固定模板）
      ★ 打上标记：commit 尾部带 "Ecode-Commit: <会话ID>"

【撤销 /undo】
用户敲 /undo
   → 检查最近一次 commit：带 Ecode-Commit 标记？
...
```
/sandbox 切换（StatusBar 常驻显示当前档位）：

┌────────────────┬──────────────┬──────────────┬────────────┐
│ 档位            │ 读文件/搜索    │ 写/改文件      │ bash 命令   │
├────────────────┼──────────────┼──────────────┼────────────┤
│ default（默认） │ ✓ 放行        │ 项目目录内 ✓   │ 每次弹确认  │
│ read-only      │ ✓ 放行        │ ✗ 直接拒绝     │ ✗ 直接拒绝  │
│ workspace-write│ ✓ 放行        │ 仅项目目录内   │ 弹确认      │
│                │              │ 越界 ✗ 拒绝   │            │
└────────────────┴──────────────┴──────────────┴────────────┘

诚实声明（软沙箱边界）：写/改文件是工具层路径检查（能拦 `..` 逃逸）；
