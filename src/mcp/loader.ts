...
//
// 职责：读 registry → 过滤 enabled → 并行 connectMcpServer → 收集 tools/connections。
// 调用方（use-agent-stream / index.ts）拿到结果后合并进 RunAgentStreamOptions.tools 即可。
// 退出时调用 disconnectAll 清理子进程。
//
// 设计：
...
export async function loadMcpServers(opts?: {
  ...
  if (connected.length > 0) {
    console.log(`[MCP] 已连接 ${connected.length} 个 server（${tools.length} 个工具）`);
  }
  if (failed.length > 0) {
