/**
 * 不可信内容净化（F-47，方案 A 批 0 前置门；B2 上移 protocol 共享层）。
 *
 * 宿主（makeToolDigest 生成即净化）与 TUI（渲染口）双侧共用单源——原居 tui/sanitize.ts，
 * 活动流 B2 批随 digest 协议化上移（tui/sanitize.ts 保留 re-export 薄壳，旧 import 不动）。
 *
