...
| 后续 M5 实施方案 | [详设/2026-08-14_后续-M5实施方案_待审核.md](./详设/2026-08-14_后续-M5实施方案_待审核.md) | 上下文压缩 v10（架构定型）：**插件化策略骨架**（学 deepseek-harness：CompactionStrategy 接口+registry+编排器+事件钩子，加策略=写插件）+ **两个默认策略**（tool-clear 免模型 + summarize 锚定，精而非多）+ **★切换 model 预检**（补两家空白：监听切换→实时重测→迭代压缩→/clear 兜底）+ 投影存储分离 + models.dev + usage 锚定；Skill/MCP→M6 |
