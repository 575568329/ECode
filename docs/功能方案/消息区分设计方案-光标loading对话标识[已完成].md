> ⚠️ **已废弃（2026-08-08）**：本方案已整合升级到 [`消息队列与交互重做方案[已完成].md`](消息队列与交互重做方案[已完成].md)（§5 对话气泡 + §6 动效，含三角色审阅修正：助手保留 ◆+左竖线、光标不闪烁、只留 thinking loader）。
> 实现以新方案为准；本文件仅作早期讨论溯源保留。

# 消息视觉区分 + 光标 / loading 动效设计方案【已废弃 → 见消息队列与交互重做方案[已完成].md】
