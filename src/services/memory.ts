 ...
 */
export function loadMemoryIndexes(opts: LoadMemoryOpts = {}): { level: 'user' | 'project'; content: string }[] {
  const out: { level: 'user' | 'project'; content: string }[] = []
    ...
    out.push({
      ...
      content:
        bytes <= MAX_MEMORY_BYTES
          ? trimmed
          : `${Buffer.from(trimmed, 'utf8').subarray(0, MAX_MEMORY_BYTES).toString('utf8')}\n[已截断：${bytes} 字节超上限]`,
    })
  }
  return out
}
